...
public:
    ...
    int WriteToFile(int fd, char *buf, int len) {
      ...
      int blockIndex=MainDir[fd]->getFsFile()->getIndexBlock();
        ...
        if(blockIndex!=-1) {
            ...
            while(i>=0){
                if (temp[i] != '\0') {
                    if ((existBlockFree = howManyByteIsClear((int) temp[i])) != 0) {
        ...
        while(done<len && MainDir[fd]->getFsFile()->getfile_size()<MAX_FILE_SIZE){
            if(existBlockFree==0){
                ...
                int dataIndexInsideBlcokIndex=blockIndex+ (sizeOfBlock-howManyByteIsClear(blockIndex));
                int ret_val = fseek(sim_disk_fd, dataIndexInsideBlcokIndex, SEEK_SET);
    ...
    int ReadFromFile(int fd, char *buf, int len) {
        ...
        //data block
        int done=0,i=0;
        while(temp[i]!='\0' && done < len && done < MAX_FILE_SIZE) {
            j=0;
            dataInd=(int)temp[i] * sizeOfBlock;
            while (j<sizeOfBlock) {
                int ret_val = fseek(sim_disk_fd, dataInd + j, SEEK_SET);
                ret_val = fread(&buf[done], 1, 1, sim_disk_fd);
                assert(ret_val == 1);
                MainDir[fd]->getFsFile()->addByte();
                done++;
                j++;
...
private:
    int howManyByteIsClear(int blockInd){
        int ind=blockInd*sizeOfBlock,counter =0;
